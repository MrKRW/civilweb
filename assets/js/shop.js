...
async function loadShopItems(container) {
  try {
    ...
    container.innerHTML = published.map(item => {
      const imgPath = item.image ? `uploads/shop/${item.image}` : 'Project images/placeholder.png';
      ...
      return `
        <div class="product-card">
          ...
          <div class="product-info">
            <h4 class="product-title"><a href="shop-product.html?id=${item.id}">${escapeHtml(item.title)}</a></h4>
